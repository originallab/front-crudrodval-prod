...
export default function Dashboard() {
  ...
  const [formData, setFormData] = useState({
    zonas_destinos: { nombre: "", descripcion: "" },
    tipos_unidades: {  nombre: "", foto_url: "", capacidad: "", descripcion: "" },
    tipos_cargas: { tipo: "", descripcion: "" },
  });

  const [selectedItem, setSelectedItem] = useState(null);
  const [activeTable, setActiveTable] = useState("zonas_destinos");
  ...
  useEffect(() => {
    const loadAllTables = async () => {
      await fetchTableData("zonas_destinos");
      await fetchTableData("tipos_unidades");
      await fetchTableData("tipos_cargas");
    };
  ...
  return (
    ...
    >
      <div className="w-full max-w-5xl bg-white rounded-2xl shadow-2xl p-8 bg-opacity-100 ml-40">
        <h1 className="text-3xl font-bold text-center mb-6">Mantenedor de datos básicos</h1>

        {/* Selector de tabla */}
        <div className="flex justify-center mb-6">
          {["zonas_destinos", "tipos_unidades", "tipos_cargas"].map((table) => (
            <button
              key={table}
              onClick={() => {
